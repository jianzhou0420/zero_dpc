...
class Transformer(Module):
    ...
    def __init__(
        ...
        encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward, dropout, activation, layer_norm_eps, batch_first, norm_first, bias, **factory_kwargs,)
        encoder_norm = LayerNorm(d_model, eps=layer_norm_eps, bias=bias, **factory_kwargs)
        self.encoder = TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)

        decoder_layer = TransformerDecoderLayer(d_model, nhead, dim_feedforward, dropout, activation, layer_norm_eps, batch_first, norm_first, bias, **factory_kwargs,)
    ...
    def forward(
        ...
        output = self.decoder(tgt, memory, tgt_mask=tgt_mask, memory_mask=memory_mask, tgt_key_padding_mask=tgt_key_padding_mask, memory_key_padding_mask=memory_key_padding_mask, tgt_is_causal=tgt_is_causal, memory_is_causal=memory_is_causal,
                              )
        return output
...
class TransformerDecoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            x = x + self._multi_head_attention_block(self.norm2(x), memory, memory_mask, memory_key_padding_mask, memory_is_causal,)
            ...
            x = self.norm2(x + self._multi_head_attention_block(x, memory, memory_mask, memory_key_padding_mask, memory_is_causal))
